...
* Heroku is used to deploy this project, finally, it will return a URL for this project where anyone else can use this URL to use our application instead of *http://localhost:3000* which is only visible for our own local machine.
...
* Inside `config/keys`, we need to develop two sets of keys: dev (inside laptop) and prod (inside Heroku) for mainly 3 reasons: 1. In case the laptop is losted or hacked; 2. For creating a new and clear MongoDB; 3. In this way, we can do development without affect the real product. In order to do this, we need to create a new set of keys (MongoDB keys, Google API keys, cookie keys...) and set them inside Heroku config variables so that they can pulled by *process.env* later on.
